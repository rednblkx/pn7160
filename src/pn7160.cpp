...
esp_err_t PN7160_NCI::initialize() {
    esp_err_t nci_status = nci::STATUS_FAILED;
    ...
    NciMessage prop_act_cmd(nci::PKT_MT_CTRL_COMMAND, nci::PROPRIETARY_GID, nci::CORE_SET_CONFIG_OID);
    NciMessage prop_act_rsp;
    nci_status = write_nci_packet(prop_act_cmd);
    if (nci_status != ESP_OK) {
        ESP_LOGE(TAG, "Error sending PROPRIETARY_GID (CORE_SET_CONFIG_OID) (err=0x%X)", nci_status);
        return nci::STATUS_FAILED;
    }

    nci_status = read_nci_packet(prop_act_rsp, nci::PN7160_INIT_TIMEOUT_MS);
    if (nci_status != ESP_OK) {
        ESP_LOGE(TAG, "Failed to read PROPRIETARY_GID (CORE_SET_CONFIG_OID) (err=0x%X)", nci_status);
        return nci::STATUS_FAILED; // Original behavior
    }
    if (!prop_act_rsp.is_control_response(nci::PROPRIETARY_GID, nci::CORE_SET_CONFIG_OID)) {
        ESP_LOGE(TAG, "Received unexpected packet after PROPRIETARY_GID (CORE_SET_CONFIG_OID) (expected NTF)");
    ...
    // Send PMU Config
    NciMessage pmu_cmd(
        nci::PKT_MT_CTRL_COMMAND, nci::CORE_GID, nci::CORE_SET_CONFIG_OID,
        std::vector<uint8_t>(std::begin(PMU_CFG), std::end(PMU_CFG)));
    NciMessage pmu_rsp;
    nci_status = write_nci_packet(pmu_cmd);
    if (nci_status != ESP_OK) {
        ESP_LOGE(TAG, "Error sending CORE_GID (CORE_SET_CONFIG_OID) (err=0x%X)", nci_status);
        return nci::STATUS_FAILED;
    }

    nci_status = read_nci_packet(pmu_rsp, nci::PN7160_INIT_TIMEOUT_MS);
    if (nci_status != ESP_OK) {
        ESP_LOGE(TAG, "Failed to read CORE_GID (CORE_SET_CONFIG_OID) (err=0x%X)", nci_status);
        return nci::STATUS_FAILED;
    }
    if (!pmu_rsp.is_control_response(nci::CORE_GID, nci::CORE_SET_CONFIG_OID)) {
        ESP_LOGE(TAG, "Received unexpected packet after CORE_GID (CORE_SET_CONFIG_OID) (expected NTF)");
    ...
    nci_status = read_nci_packet(duration_rsp, nci::PN7160_INIT_TIMEOUT_MS);
    if (nci_status != ESP_OK) {
        ESP_LOGE(TAG, "Failed to read CORE_GID (CORE_SET_CONFIG_OID) (err=0x%X)", nci_status);
        return nci::STATUS_FAILED; // Original behavior
    }
    if (!duration_rsp.is_control_response(nci::CORE_GID, nci::CORE_SET_CONFIG_OID)) {
        ESP_LOGE(TAG, "Received unexpected packet after CORE_GID (CORE_SET_CONFIG_OID) (expected NTF)");
...
esp_err_t PN7160_NCI::hardware_reset() {
    ...
    vTaskDelay(pdMS_TO_TICKS(
        nci::PN7160_INIT_TIMEOUT_MS)); // Wait for chip to boot (UM11495 Tboot=5ms)
 ...
 * @return esp_err_t nci::STATUS_OK on success (APDU exchange completed, check r_apdu status word),
...
esp_err_t PN7160_NCI::send_apdu_sync(const std::vector<uint8_t> &c_apdu,
  ...
  sync_apdu_in_progress_.store(true);
  ...
  if (send_ret != ESP_OK) {
    ...
    if (xSemaphoreTake(sync_mutex_, pdMS_TO_TICKS(50)) == pdTRUE) {
      sync_apdu_in_progress_.store(false);
      xSemaphoreGive(sync_mutex_);
    }
    return result;
  }
    ...
    if (xSemaphoreTake(sync_mutex_, pdMS_TO_TICKS(50)) == pdTRUE) {
      if (!sync_apdu_response_.empty()) {
        ...
        ESP_LOGD(TAG, "send_apdu_sync: R-APDU received (%zu bytes)",
                 r_apdu.size());
        result = nci::STATUS_OK; // NCI exchange successful
      } else {
        ESP_LOGE(
            TAG,
      ...
      sync_apdu_in_progress_.store(false); // Mark operation as finished
      xSemaphoreGive(sync_mutex_);
    } else {
      ...
      sync_apdu_in_progress_.store(false); // Attempt to clear anyway
    }
  } else {
...
esp_err_t PN7160_NCI::core_reset(bool reset_config) {
    if(transport.has_ven()){
      transport.set_ven(false);
      vTaskDelay(pdMS_TO_TICKS(nci::PN7160_DEFAULT_TIMEOUT_MS)); // Reset pulse width
      transport.set_ven(true);
      vTaskDelay(pdMS_TO_TICKS(nci::PN7160_DEFAULT_TIMEOUT_MS)); // Wait for boot
      ESP_LOGD(TAG, "Power cycle reset complete.");
    }
    esp_err_t read_status_ntf;
    if (transport.read_irq_level()) {
        ESP_LOGW(TAG, "IRQ high after power cycle, clearing pending message");
        NciMessage dummy_msg;
        read_status_ntf = read_nci_packet(dummy_msg, 50);
    }

    // NCI 2.0 Sequence: CMD -> RSP -> NTF
    NciMessage cmd(nci::PKT_MT_CTRL_COMMAND, nci::CORE_GID, nci::CORE_RESET_OID,
    ...
    read_status_ntf = read_nci_packet(rsp, nci::PN7160_INIT_TIMEOUT_MS);
    ...
    read_status_ntf =
        read_nci_packet(ntf, nci::PN7160_INIT_TIMEOUT_MS);
    if (read_status_ntf != ESP_OK) {
        ESP_LOGE(TAG, "Failed to read CORE_RESET_NTF (err=0x%X)", read_status_ntf);
        return nci::STATUS_FAILED;
    }
    ESP_LOGD(TAG, "Received CORE_RESET_RSP, checking NTF...");
...
esp_err_t PN7160_NCI::core_init() {
    NciMessage cmd(nci::PKT_MT_CTRL_COMMAND, nci::CORE_GID, nci::CORE_INIT_OID);
    NciMessage rsp;
    ...
    // NCI Core Init RSP payload layout (NCI 2.0 Section 7.1.2):
    // Byte 0: Status, Byte 1: NFCC Features (len=4), Byte 5+: data
    if (rsp.size() > 20) {
...
void PN7160_NCI::task_runner() {
    ...
    while (!stop_flag_.load()) {
        if ((ret = transport.wait_for_irq(true, pdMS_TO_TICKS(100))) == ESP_OK) {
                ...
                if(!processed_synchronously){
                    switch (incoming_msg.get_mt()) {
                        case nci::PKT_MT_CTRL_NOTIFICATION:
                            ...
                            if (incoming_msg.get_gid() == nci::RF_GID) {
                                switch (incoming_msg.get_oid()) {
                                    ...
                                    break;
                                    case nci::RF_ISO_DEP_NAK_PRESENCE_OID: {  // CTRL_NOTIFICATION
                                        esp_log_buffer_hexdump_internal(TAG, incoming_msg.raw_data(), incoming_msg.size(), ESP_LOG_DEBUG);
                                        uint8_t presence = incoming_msg[3];
                                        esp_err_t ret;
                                        if (presence != 0x00) {
                                            ESP_LOGW(TAG, "Tag no longer in field");
                                            selected_tag_still_in_field.store(false);
                                            ret = rf_deactivate(nci::DEACTIVATION_TYPE_DISCOVERY);
                                        } else {
                                            ESP_LOGD(TAG, "Tag still in field");
                                            ret = rf_iso_dep_presence_check();
                                        }
                                        break;
                                    }
                                    default:
                            ...
                            } else if (incoming_msg.get_gid() == nci::CORE_GID) {
                                switch (incoming_msg.get_oid()) {
        ...
        } else if(ret != ESP_ERR_TIMEOUT) { // wait_for_irq failed
